...
MODEL_TYPE = "yolo"
REPO_ID = "HyunchanAn/Dental_012"
PT_PATH = "weights/best.pt" # Update path if different

def main():
    if not os.path.exists(PT_PATH):
        print(f"[Dental_012] No .pt file found at {PT_PATH}.")
        return

    print(f"[Dental_012] Exporting {PT_PATH} to ONNX...")
    ...
    if onnx_path and os.path.exists(onnx_path):
        print(f"[Dental_012] Uploading to HuggingFace {REPO_ID}...")
        api = HfApi()
        ...
        try:
            ...
            print(f"[Dental_012] Upload successful. Deleting local weights to save space...")
            os.remove(PT_PATH)
            os.remove(onnx_path)
            print(f"[Dental_012] Cleaned up local disk.")
        except Exception as e:
            print(f"[Dental_012] Upload failed: {e}")
